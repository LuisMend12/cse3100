...
void *producer(void *arg) {
  ...
  while (1) {
    ...
    while (count == BUFFER_SIZE) { //the buffer is full
    ...
    pthread_cond_signal(&cond_consumer);//this a signal to the consumer that the thread has finished.
...
void *consumer(void *arg) {
  ...
  while (1) {
    ...
    pthread_cond_signal(&cond_producer); //this is the signal for consumer
    pthread_mutex_unlock(&mutex);//this is the signal for the producer
